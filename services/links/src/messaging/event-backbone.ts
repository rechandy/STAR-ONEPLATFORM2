 ...
 *  - onModuleInit: connect the producer/consumer
...
export class EventBackbone implements OnModuleInit, OnApplicationBootstrap, OnModuleDestroy {
  ...
  constructor(config: ConfigService) {
    ...
    if (this.mode === 'kafka') {
      ...
      const kafka = new Kafka({ clientId: config.get<string>('kafkaClientId', 'links'), brokers });
      this.producer = kafka.producer();
      this.consumer = kafka.consumer({ groupId: config.get<string>('kafkaGroupId', 'links') });
