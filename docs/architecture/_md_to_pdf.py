#!/usr/bin/env python3
"""
Render a markdown doc to a styled PDF. Any Mermaid code blocks are rasterized
to PNG (via mermaid.ink) and embedded; otherwise it's a pure local conversion
(markdown -> HTML -> xhtml2pdf), no headless browser required.

  python docs/architecture/_md_to_pdf.py                       # the architecture doc
  python docs/architecture/_md_to_pdf.py --src path.md --out path.pdf
  python docs/architecture/_md_to_pdf.py --landscape --src ... # wide tables
"""
import argparse
import base64
import json
...
DEFAULT_SRC = HERE / "06-prototype-architecture.md"

CSS_TMPL = """
@page {{ size: {size}; margin: 1.6cm 1.5cm; }}
...
pre {{ background: #f1f5fa; padding: 5pt; font-size: 8pt; }}
strong {{ color: #0a2540; }}
...
blockquote {{ color: #5b6b7b; font-style: italic; border-left: 2pt solid #cdd7e2; padding-left: 8pt; margin: 6pt 0; }}
hr {{ border: none; border-top: 0.5pt solid #d7e0ea; }}
"""


def render_mermaid(diagram: str, client) -> bytes:
...
def main() -> None:
    ...
    css = CSS_TMPL.format(size="letter landscape" if args.landscape else "letter")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if blocks:
            import httpx
            import truststore
            print(f"Rendering {len(blocks)} Mermaid diagram(s) via mermaid.ink …")
            ctx = truststore.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
            with httpx.Client(verify=ctx, timeout=httpx.Timeout(120.0)) as client:
                for i, blk in enumerate(blocks):
                    p = td / f"diagram_{i}.png"
                    p.write_bytes(render_mermaid(blk, client))
                    png_paths.append(p)
                    print(f"  diagram {i + 1}: {p.stat().st_size // 1024} KB")
        ...
        for i, p in enumerate(png_paths):
            html_body = html_body.replace(
                f"[[DIAGRAM_{i}]]", f'<div><img class="diagram" src="{p.as_posix()}"/></div>'
            )

        html = f"<html><head><style>{css}</style></head><body>{html_body}</body></html>"
        with open(out, "wb") as f:
            result = pisa.CreatePDF(html, dest=f, link_callback=lambda uri, rel: uri, encoding="utf-8")
        if result.err:
